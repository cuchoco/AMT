...
class TverskyLoss(_Loss):
    ...
    def __init__(
        ...
        self.batch = batch
        self.loss_smooth_alpha = loss_smooth_alpha
    ...
    def forward(self, input: torch.Tensor, target: torch.Tensor) -> torch.Tensor:
        ...
        if self.loss_smooth_alpha is not None:
            score = torch.exp(-self.loss_smooth_alpha * score)
        else:
...
class TverskyLossWithCEBatch(nn.Module):
    def __init__(self, alpha=0.3, smooth=1e-4, loss_smooth_alpha=None): # for less false negatives increase alpha
        """
        alpha: weight for false positives
        beta: weight for false negatives
        smooth: smoothing constant to prevent division by zero
        """
        super(TverskyLossWithCEBatch, self).__init__()
        ...
        self.dice = TverskyLoss(
                        ...
                        smooth_nr=smooth,
                        batch=True,
                        loss_smooth_alpha=loss_smooth_alpha
                        )

    def forward(self, preds, targets):
        # Compute CE loss
        targets = targets.float()
        ce_loss = F.binary_cross_entropy_with_logits(preds, targets, reduction='mean') * 0.1
