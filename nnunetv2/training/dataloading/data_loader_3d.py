...
class nnUNetDataLoader3D(nnUNetDataLoaderBase):
    def generate_train_batch(self):
        ...
        is_abnormal = np.zeros((self.batch_size, 1), dtype=np.int16)
        case_properties = []
        ...
        if self.transforms is not None:
            with torch.no_grad():
                with threadpool_limits(limits=1, user_api=None):
                    ...
                    for b in range(self.batch_size):
                        ...
                        is_abnormal[b] = 1 if torch.sum(segs[-1][0] > 0) > 10 else 0
            ...
            return {'data': data_all, 'target': seg_all, 'keys': selected_keys, 'is_abnormal': torch.LongTensor(is_abnormal)}
